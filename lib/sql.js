...
function leerConfig() {
    ...
    const cfgEscala = require("./config").leer();
    return {
        servidor: process.env.SQL_SERVER || tag(xml, "servidor") || cfgEscala.servidor || "localhost",
        base: process.env.SQL_DATABASE || tag(xml, "base") || cfgEscala.base || "001_v2v_gps",
        autenticacion: (process.env.SQL_AUTH || tag(xml, "autenticacion") || cfgEscala.autenticacion || "windows").toLowerCase(),
        usuario: process.env.SQL_USER || tag(xml, "usuario") || cfgEscala.usuario,
        clave: process.env.SQL_PASSWORD || tag(xml, "clave") || cfgEscala.clave
    };
}
...
async function conectar() {
    const nube = require("./config").enNube;
    const loop = require("./config").esLoopback;
    if (nube() && !process.env.SQL_SERVER) {
        console.log("Plan B: sin SQL Server (Railway no lo tiene). Se usa JSON.");
        listo = false;
        return false;
    }
    try {
        ...
        console.error("SQL config:", err && err.message ? err.message : err);
        listo = false;
        return false;
    }
    if (nube() && loop(cfgCache.servidor)) {
        console.log("SQL Server local omitido en Railway; se usa JSON.");
        listo = false;
        return false;
    }
    ...
    if (process.platform === "win32" && await conectarSqlcmd(cfgCache)) {
        modo = "sqlcmd";
        listo = true;
